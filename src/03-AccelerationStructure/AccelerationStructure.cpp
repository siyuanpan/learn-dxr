...
void AccelerationStructure::initDXR(HWND winHandle, uint32_t winWidth,
                                    uint32_t winHeight) {
  mHwnd = winHandle;
...
uint32_t AccelerationStructure::beginFrame() {
  return mpSwapChain->GetCurrentBackBufferIndex();
}

void AccelerationStructure::endFrame(uint32_t rtvIndex) {
  ...
  if (mFenceValue > kDefaultSwapChainBuffers) {
    mpFence->SetEventOnCompletion(mFenceValue - kDefaultSwapChainBuffers + 1,
                                  mFenceEvent);
...
ID3D12ResourcePtr createBuffer(ID3D12Device5Ptr pDevice, uint64_t size,
  ...
  d3d_call(pDevice->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE,
...
ID3D12ResourcePtr createTriangleVB(ID3D12Device5Ptr pDevice) {
  ...
  ID3D12ResourcePtr pBuffer =
      createBuffer(pDevice, sizeof(vertices), D3D12_RESOURCE_FLAG_NONE,
...
AccelerationStructureBuffers createBottomLevelAS(
    ID3D12Device5Ptr pDevice, ID3D12GraphicsCommandList4Ptr pCmdList,
  ...
  buffers.pScratch =
      createBuffer(pDevice, info.ScratchDataSizeInBytes,
                   D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS, kDefaultHeapProps);
  buffers.pResult =
      createBuffer(pDevice, info.ResultDataMaxSizeInBytes,
                   D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
  ...
  asDesc.DestAccelerationStructureData =
      buffers.pResult->GetGPUVirtualAddress();
  asDesc.ScratchAccelerationStructureData =
      buffers.pScratch->GetGPUVirtualAddress();
...
AccelerationStructureBuffers createTopLevelAS(
  ...
  buffers.pInstanceDesc = createBuffer(
      pDevice, sizeof(D3D12_RAYTRACING_INSTANCE_DESC), D3D12_RESOURCE_FLAG_NONE,
      D3D12_RESOURCE_STATE_GENERIC_READ, kUploadHeapProps);
...
void AccelerationStructure::createAccelerationStructures() {
  mpVertexBuffer = createTriangleVB(mpDevice);
  AccelerationStructureBuffers bottomLevelBuffers =
      createBottomLevelAS(mpDevice, mpCmdList, mpVertexBuffer);
  AccelerationStructureBuffers topLevelBuffers = createTopLevelAS(
      mpDevice, mpCmdList, bottomLevelBuffers.pResult, mTlasSize);
...
void AccelerationStructure::onLoad(HWND winHandle, uint32_t winWidth,
                                   uint32_t winHeight) {
  initDXR(winHandle, winWidth, winHeight);
  createAccelerationStructures();
}

void AccelerationStructure::onFrameRender() {
...
void AccelerationStructure::onShutdown() {
  mFenceValue++;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  Framework::run(AccelerationStructure(), "03 - Acceleration Structure");
}